...
public class CBDatabase {
    ...
    private class CBSchemaVersionManager implements SQLSchemaVersionManager {
        ...
        public int getCurrentSchemaVersion(DBRProgressMonitor monitor, Connection connection, String schemaName) throws DBException, SQLException {
            ...
            try {
                int version = CommonUtils.toInt(JDBCUtils.executeQuery(connection,
                    "SELECT VERSION FROM " + getTableName(CBDatabaseConstants.TABLE_CB_SCHEMA_INFO)));
                return version == 0 ? 1 : version;
            } catch (SQLException e) {
                try {
                    Object legacyVersion = CommonUtils.toInt(JDBCUtils.executeQuery(connection,
                        "SELECT SCHEMA_VERSION FROM " + getTableName(CBDatabaseConstants.TABLE_CB_SERVER)));
        ...
        public void updateCurrentSchemaVersion(
            ...
            var updateCount = JDBCUtils.executeUpdate(
                connection,
                "UPDATE " + getTableName(CBDatabaseConstants.TABLE_CB_SCHEMA_INFO) + " SET VERSION=?,UPDATE_TIME=CURRENT_TIMESTAMP",
                version
            );
            if (updateCount <= 0) {
                JDBCUtils.executeSQL(
                    connection,
                    "INSERT INTO " + getTableName(CBDatabaseConstants.TABLE_CB_SCHEMA_INFO) +
                        "(VERSION,UPDATE_TIME) VALUES(?,CURRENT_TIMESTAMP)",
                    version
                );
            }
        }
    ...
    private void checkInstanceRecord(Connection connection) throws SQLException, IOException {
        ...
        boolean hasInstanceRecord = JDBCUtils.queryString(connection,
            "SELECT HOST_NAME FROM " + getTableName(CBDatabaseConstants.TABLE_CB_INSTANCE) +
                " WHERE INSTANCE_ID=?", instanceId) != null;
        if (!hasInstanceRecord) {
            JDBCUtils.executeSQL(
                connection,
                "INSERT INTO " + getTableName(CBDatabaseConstants.TABLE_CB_INSTANCE) +
                    "(INSTANCE_ID,MAC_ADDRESS,HOST_NAME,PRODUCT_NAME,PRODUCT_VERSION,UPDATE_TIME) VALUES(?,?,?,?,?,CURRENT_TIMESTAMP)",
                instanceId,
                macAddress,
                hostName,
                productName,
            ...
            JDBCUtils.executeSQL(
                connection,
                "UPDATE "+ getTableName(CBDatabaseConstants.TABLE_CB_INSTANCE) +
                    " SET HOST_NAME=?,PRODUCT_NAME=?,PRODUCT_VERSION=?,UPDATE_TIME=CURRENT_TIMESTAMP WHERE INSTANCE_ID=?",
                hostName,
                productName,
                versionName,
                instanceId);
        }
        JDBCUtils.executeSQL(connection, "DELETE FROM " + getTableName(CBDatabaseConstants.TABLE_CB_INSTANCE_DETAILS) +
        ...
        try (PreparedStatement dbStat = connection.prepareStatement(
            "INSERT INTO " + getTableName(CBDatabaseConstants.TABLE_CB_INSTANCE_DETAILS) +
            "(INSTANCE_ID,FIELD_NAME,FIELD_VALUE) VALUES(?,?,?)")) {
    ...
    @NotNull
    public String getTableName(@NotNull String name) {
        if (databaseConfiguration.getSchema() == null) {
            return name;
        }
        return databaseConfiguration.getSchema() + "." + name;
    }
