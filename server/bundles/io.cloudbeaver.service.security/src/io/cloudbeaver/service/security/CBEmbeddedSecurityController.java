    ...
    protected SMAuthInfo finishAuthentication(
        ...
        for (SMAuthConfigurationReference authConfiguration : authProviderIds) {
            ...
            if (isMainAuthSession) {
                ...
                if (autoAssign != null && CommonUtils.isNotEmpty(autoAssign.getAuthRoleChangedReason())) {
                    log.info(activeUserId + " authenticated with role " + autoAssign.getAuthRole() + ", reason: " + autoAssign.getAuthRoleChangedReason());
                }
            }
