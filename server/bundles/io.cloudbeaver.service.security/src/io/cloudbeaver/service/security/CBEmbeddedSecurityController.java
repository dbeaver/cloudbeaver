...
import io.cloudbeaver.service.security.db.CBDatabaseConstants;
...
public class CBEmbeddedSecurityController implements SMAdminController, SMAuthenticationManager {
    ...
    private boolean isSubjectExists(String subjectId) throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement("SELECT 1 FROM " +
                database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_SUBJECT) + " WHERE SUBJECT_ID=?")) {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                ...
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "INSERT INTO " + 
                        database.getTableName(CBDatabaseConstants.TABLE_CB_USER) + 
    ...
    public void deleteUser(String userId) throws DBCException {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                ...
                JDBCUtils.executeStatement(dbCon, "DELETE FROM " +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_USER) + " WHERE USER_ID=?", userId);
                txn.commit();
            }
        } catch (SQLException e) {
    ...
    public void setUserTeams(String userId, String[] teamIds, String grantorId) throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                JDBCUtils.executeStatement(dbCon, "DELETE FROM " +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) + " WHERE USER_ID=?", userId);
                if (!ArrayUtils.isEmpty(teamIds)) {
                    try (PreparedStatement dbStat = dbCon.prepareStatement("INSERT INTO " +
                        database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) +
    ...
    public SMTeam[] getUserTeams(String userId) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT R.* FROM " +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) + " UR," +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_TEAM) + " R " +
    ...
    private Set<String> getAllLinkedSubjects(Connection dbCon, String subjectId) throws SQLException {
        ...
        try (PreparedStatement dbStat = dbCon.prepareStatement("SELECT TEAM_ID FROM " +
            database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) + " UR WHERE USER_ID=?")) {
            dbStat.setString(1, subjectId);
            try (ResultSet dbResult = dbStat.executeQuery()) {
                while (dbResult.next()) {
    ...
    public SMUser getUserById(String userId) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            SMUser user;
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT USER_ID,IS_ACTIVE,DEFAULT_AUTH_ROLE FROM " +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_USER) + " WHERE USER_ID=?"
            )) {
                dbStat.setString(1, userId);
                try (ResultSet dbResult = dbStat.executeQuery()) {
                    if (dbResult.next()) {
            ...
            // Teams
            try (PreparedStatement dbStat = dbCon.prepareStatement("SELECT TEAM_ID FROM " +
                database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) + " WHERE USER_ID=?")) {
                dbStat.setString(1, userId);
                try (ResultSet dbResult = dbStat.executeQuery()) {
    ...
    public SMUser[] findUsers(String userNameMask) throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            ...
            // Read teams
            try (PreparedStatement dbStat = dbCon.prepareStatement("SELECT USER_ID,TEAM_ID FROM " +
                database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) +
    ...
    private void cleanupSubjectMeta(Connection dbCon, String subjectId) throws SQLException {
        // Delete old metas
        try (PreparedStatement dbStat = dbCon.prepareStatement(
            "DELETE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_SUBJECT_META) + " WHERE SUBJECT_ID=?")) {
            dbStat.setString(1, subjectId);
            dbStat.execute();
        }
    }
    ...
    private void readSubjectMetas(Connection dbCon, SMSubject subject) throws SQLException {
        // Metas
        try (PreparedStatement dbStat = dbCon.prepareStatement(
            "SELECT META_ID,META_VALUE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_SUBJECT_META) +
                " WHERE SUBJECT_ID=?")) {
    ...
    private void readSubjectsMetas(Connection dbCon, SMSubjectType subjectType, String nameMask, Map<String, ? extends SMSubject> result) throws SQLException {
        // Read metas
        try (PreparedStatement dbStat = dbCon.prepareStatement(
            "SELECT m.SUBJECT_ID,m.META_ID,m.META_VALUE FROM " + 
                database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_SUBJECT) + " s, " +
                database.getTableName(CBDatabaseConstants.TABLE_CB_SUBJECT_META) + " m\n" +
    ...
    private void saveSubjectMetas(Connection dbCon, String subjectId, Map<String, String> metaParameters) throws SQLException {
        if (!CommonUtils.isEmpty(metaParameters)) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "INSERT INTO " + 
                    database.getTableName(CBDatabaseConstants.TABLE_CB_SUBJECT_META) +
                    "(SUBJECT_ID,META_ID,META_VALUE) VALUES(?,?,?)")) {
                dbStat.setString(1, subjectId);
    ...
    public Map<String, Object> getCurrentUserParameters() throws DBCException {
        ...
        try (Connection dbCon = database.openConnection()) {
            ...
            // Read users
            try (PreparedStatement dbStat = dbCon.prepareStatement("SELECT * FROM " + 
                database.getTableName(CBDatabaseConstants.TABLE_CB_USER_PARAMETERS) + 
                " WHERE USER_ID=?")) {
                dbStat.setString(1, userId);
                try (ResultSet dbResult = dbStat.executeQuery()) {
                    while (dbResult.next()) {
    ...
    public void setCurrentUserParameter(String name, Object value) throws DBException {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                if (value == null) {
                    // Delete old metas
                    try (PreparedStatement dbStat = dbCon.prepareStatement("DELETE FROM " + 
                        database.getTableName(CBDatabaseConstants.TABLE_CB_USER_PARAMETERS) + 
                        " WHERE USER_ID=? AND PARAM_ID=?")) {
                        dbStat.setString(1, userId);
                    ...
                    boolean updated;
                    try (PreparedStatement dbStat = dbCon.prepareStatement("UPDATE " + 
                        database.getTableName(CBDatabaseConstants.TABLE_CB_USER_PARAMETERS) + 
    ...
    public void setUserAuthRole(@NotNull String userId, @Nullable String authRole) throws DBException {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement("UPDATE " + 
                database.getTableName(CBDatabaseConstants.TABLE_CB_USER) + 
                " SET DEFAULT_AUTH_ROLE=? WHERE USER_ID=?")) {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                JDBCUtils.executeStatement(
                    dbCon,
                    "DELETE FROM " +
                        database.getTableName(CBDatabaseConstants.TABLE_CB_USER_CREDENTIALS) +
                ...
                if (!CommonUtils.isEmpty(credentials)) {
                    try (PreparedStatement dbStat = dbCon.prepareStatement(
                        "INSERT INTO " + database.getTableName(CBDatabaseConstants.TABLE_CB_USER_CREDENTIALS) + 
    ...
    private String findUserByCredentials(WebAuthProviderDescriptor authProvider, Map<String, Object> authParameters) throws DBCException {
        ...
        sql.append("SELECT U.USER_ID,U.IS_ACTIVE FROM ")
            .append(database.getTableName(CBDatabaseConstants.TABLE_CB_USER)).append(" U,")
            .append(database.getTableName(CBDatabaseConstants.TABLE_CB_USER_CREDENTIALS)).append(" UC\n");
        for (int joinNum = 0; joinNum < identCredentials.size() - 1; joinNum++) {
            sql.append(",")
                .append(database.getTableName(CBDatabaseConstants.TABLE_CB_USER_CREDENTIALS))
                .append(" UC")
                .append(joinNum + 2);
        }
    ...
    public Map<String, Object> getUserCredentials(String userId, String authProviderId) throws DBCException {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT CRED_ID,CRED_VALUE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_USER_CREDENTIALS) + "\n" +
                    "WHERE USER_ID=? AND PROVIDER_ID=?")) {
                dbStat.setString(1, userId);
    ...
    public SMTeam[] readAllTeams() throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            ...
            try (Statement dbStat = dbCon.createStatement()) {
                try (ResultSet dbResult = dbStat.executeQuery("SELECT * FROM " + 
                    database.getTableName(CBDatabaseConstants.TABLE_CB_TEAM) + " ORDER BY TEAM_ID")) {
                    while (dbResult.next()) {
                ...
                try (ResultSet dbResult = dbStat.executeQuery("SELECT SUBJECT_ID,PERMISSION_ID\n" +
                    "FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PERMISSIONS) + " AP," +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_TEAM) + " R\n" +
    ...
    public String[] getTeamMembers(String teamId) throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT USER_ID FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) + " WHERE TEAM_ID=?")) {
    ...
    public void createTeam(String teamId, String name, String description, String grantor) throws DBCException {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                ...
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "INSERT INTO " + database.getTableName(CBDatabaseConstants.TABLE_CB_TEAM) +
    ...
    public void updateTeam(String teamId, String name, String description) throws DBCException {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "UPDATE " + database.getTableName(CBDatabaseConstants.TABLE_CB_TEAM) +
                        " SET TEAM_NAME=?,TEAM_DESCRIPTION=? WHERE TEAM_ID=?")) {
    ...
    public void deleteTeam(String teamId) throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT COUNT(*) FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) + " WHERE TEAM_ID=?")) {
                dbStat.setString(1, teamId);
                try (ResultSet dbResult = dbStat.executeQuery()) {
                    if (dbResult.next()) {
            ...
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                ...
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "DELETE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_TEAM) + " WHERE TEAM_ID=?")) {
                    dbStat.setString(1, teamId);
                    dbStat.execute();
                }
                txn.commit();
...
//        validatePermissions(SMConstants.SUBJECT_PERMISSION_SCOPE, permissionIds);
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                JDBCUtils.executeStatement(dbCon, "DELETE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PERMISSIONS) +
    ...
    private void insertPermissions(Connection dbCon, String subjectId, String[] permissionIds, String grantorId) throws SQLException {
        if (!ArrayUtils.isEmpty(permissionIds)) {
            try (PreparedStatement dbStat = dbCon.prepareStatement("INSERT INTO " +
                database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PERMISSIONS) +
    ...
    public Set<String> getSubjectPermissions(String subjectId) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            ...
            try (PreparedStatement dbStat = dbCon.prepareStatement("SELECT PERMISSION_ID FROM " +
                database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PERMISSIONS) +
                " WHERE SUBJECT_ID=?")) {
                dbStat.setString(1, subjectId);
                try (ResultSet dbResult = dbStat.executeQuery()) {
    ...
    public Set<String> getUserPermissions(String userId) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            Set<String> permissions = new HashSet<>();
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT DISTINCT AP.PERMISSION_ID FROM " +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PERMISSIONS) + " AP," +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_USER_TEAM) + " UR\n" +
            ...
            try (PreparedStatement dbStat = dbCon.prepareStatement("SELECT PERMISSION_ID FROM " +
                database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PERMISSIONS) + " WHERE SUBJECT_ID=?")) {
                dbStat.setString(1, userId);
                try (ResultSet dbResult = dbStat.executeQuery()) {
                    while (dbResult.next()) {
                        permissions.add(dbResult.getString(1));
    ...
    public boolean isSessionPersisted(String id) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT 1 FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_SESSION) + " WHERE SESSION_ID=?")) {
    ...
    private String createSmSession(
        ...
        try (PreparedStatement dbStat = dbCon.prepareStatement(
            "INSERT INTO " + database.getTableName(CBDatabaseConstants.TABLE_CB_SESSION)
                + "(SESSION_ID, APP_SESSION_ID, USER_ID,CREATE_TIME,LAST_ACCESS_TIME,"
    ...
    private String createNewAuthAttempt(
        ...
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "INSERT INTO " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT) +
                ...
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "INSERT INTO " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT_INFO) +
    ...
    private void updateAuthStatus(
        ...
        try (Connection dbCon = database.openConnection(); JDBCTransaction txn = new JDBCTransaction(dbCon)) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "UPDATE " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT) +
                    " SET AUTH_STATUS=?,AUTH_ERROR=?,SESSION_ID=? WHERE AUTH_ID=?")) {
            ...
            for (Map.Entry<SMAuthConfigurationReference, Object> entry : authInfo.entrySet()) {
                ...
                sqlBuilder.append("UPDATE " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT_INFO) + " SET AUTH_STATE=? ")
    ...
    private SMAuthInfo getAuthStatus(@NotNull String authId, boolean readExpiredData) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            ...
            String smSessionId;
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT AUTH_STATUS,AUTH_ERROR,SESSION_ID FROM " +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT) + " WHERE AUTH_ID=?"
            )) {
            ...
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT AUTH_PROVIDER_ID,AUTH_PROVIDER_CONFIGURATION_ID,AUTH_STATE " +
                    "FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT_INFO) +
    ...
    private List<SMAuthInfo> findAllSmSessionAuthData(String smSessionId) throws DBException {
        ...
        try (var dbCon = database.openConnection()) {
            List<String> authAttemptIds = JDBCUtils.queryStrings(dbCon,
                "SELECT AUTH_ID FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT) +
    ...
    private void invalidateUserTokens(String smToken) throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            JDBCUtils.executeStatement(dbCon,
                "DELETE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) + " WHERE TOKEN_ID=?", smToken);
        } catch (SQLException e) {
    ...
    private void invalidateAllUserTokens(String userId) throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            JDBCUtils.executeStatement(dbCon,
                "DELETE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) + " WHERE USER_ID=?", userId);
        } catch (SQLException e) {
            throw new DBCException("Session invalidation failed", e);
        }
    }
    ...
    private SMTokens findTokenBySmSession(String smSessionId) throws DBException {
        try (Connection dbCon = database.openConnection();
             PreparedStatement dbStat = dbCon.prepareStatement("SELECT TOKEN_ID, REFRESH_TOKEN_ID FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) + " WHERE SESSION_ID=?")
        ) {
    ...
    private SMTokenInfo findTokenByAppSession(@NotNull String appSessionId) throws DBException {
        try (var dbCon = database.openConnection();
             var dbStat = dbCon.prepareStatement(
                 "SELECT CAT.TOKEN_ID FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) + " CAT " +
                     "  JOIN " + database.getTableName(CBDatabaseConstants.TABLE_CB_SESSION) + " CS ON CAT.SESSION_ID = CS.SESSION_ID " +
    ...
    private SMTokenInfo readAccessTokenInfo(String smAccessToken) throws DBException {
        try (Connection dbCon = database.openConnection();
             PreparedStatement dbStat = dbCon.prepareStatement(
                 "SELECT REFRESH_TOKEN_ID,SESSION_ID,USER_ID,REFRESH_TOKEN_EXPIRATION_TIME,AUTH_ROLE FROM " +
                     database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) + " WHERE TOKEN_ID=?"
             )
        ) {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT AUTH_PROVIDER_CONFIGURATION_ID " +
                    " FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT) + "_INFO "
    ...
    private AuthAttemptSessionInfo readAuthAttemptSessionInfo(@NotNull String authId) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT APP_SESSION_ID,SESSION_TYPE,APP_SESSION_STATE,SESSION_ID FROM " +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT) + " WHERE AUTH_ID=?"
            )) {
                dbStat.setString(1, authId);
                try (ResultSet dbResult = dbStat.executeQuery()) {
    ...
    protected SMTokens generateNewSessionToken(
        ...
        JDBCUtils.executeStatement(dbCon, "DELETE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) +
            " WHERE SESSION_ID=?", smSessionId);
    ...
    private SMTokens generateNewSessionTokens(
        ...
        try (PreparedStatement dbStat = dbCon.prepareStatement(
            "INSERT INTO " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) +
    ...
    private SMAuthPermissions getTokenPermissions(@NotNull String token) throws DBException {
        ...
        try (Connection dbCon = database.openConnection();
             PreparedStatement dbStat = dbCon.prepareStatement(
                 "SELECT USER_ID, EXPIRATION_TIME, SESSION_ID, AUTH_ROLE FROM " +
                     database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) + " WHERE TOKEN_ID=?");
        ) {
    ...
    public void updateSession(@NotNull String sessionId, @NotNull Map<String, Object> parameters) throws DBCException {
        ...
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "UPDATE " + 
                    database.getTableName(CBDatabaseConstants.TABLE_CB_SESSION) + 
...
//        validatePermissions(objectType.getObjectType(), permissions);
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                var sqlBuilder = new StringBuilder("DELETE FROM ")
                    .append(database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS))
                ...
                if (!CommonUtils.isEmpty(permissions)) {
                    try (PreparedStatement dbStat = dbCon.prepareStatement(
                        "INSERT INTO " + 
                            database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS) + 
    ...
    public void deleteAllObjectPermissions(@NotNull String objectId, @NotNull SMObjectType objectType) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            JDBCUtils.executeStatement(dbCon,
                "DELETE FROM " + 
                    database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS) + " WHERE OBJECT_TYPE=? AND OBJECT_ID=?",
    ...
    public void deleteAllSubjectObjectPermissions(@NotNull String subjectId, @NotNull SMObjectType objectType) throws DBException {
        try (Connection dbCon = database.openConnection()) {
            JDBCUtils.executeStatement(dbCon,
                "DELETE FROM " + 
                    database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS) + " WHERE OBJECT_TYPE=? AND SUBJECT_ID=?",
                objectType.getObjectType(),
                subjectId
            );
    ...
    public List<SMObjectPermissions> getAllAvailableObjectsPermissions(@NotNull SMObjectType objectType) throws DBException {
        ...
        try (Connection dbCon = database.openConnection()) {
            ...
            {
                var sqlBuilder = new StringBuilder("SELECT OBJECT_ID,PERMISSION FROM ")
                    .append(database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS));
    ...
    public SMObjectPermissions getObjectPermissions(
        ...
        try (Connection dbCon = database.openConnection()) {
            Set<String> allSubjects = getAllLinkedSubjects(dbCon, subjectId);
            {
                var sqlBuilder = new StringBuilder("SELECT PERMISSION FROM ")
                    .append(database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS));
                sqlBuilder.append(" WHERE SUBJECT_ID IN (");
                appendStringParameters(sqlBuilder, allSubjects);
        ...
        try (Connection dbCon = database.openConnection()) {
            try (PreparedStatement dbStat = dbCon.prepareStatement(
                "SELECT OP.SUBJECT_ID,S.SUBJECT_TYPE, OP.PERMISSION\n" +
                    "FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS) + " OP," +
                    database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_SUBJECT) + " S\n" +
        ...
        try (Connection dbCon = database.openConnection()) {
            ...
            var sqlBuilder =
                new StringBuilder("SELECT OP.OBJECT_ID,S.SUBJECT_TYPE,S.SUBJECT_ID,OP.PERMISSION\n")
                    .append("FROM ")
                    .append(database.getTableName(CBDatabaseConstants.TABLE_CB_OBJECT_PERMISSIONS)).append(" OP,")
                    .append(database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_SUBJECT)).append(" S\n")
    ...
    protected String readTokenAuthRole(String smAccessToken) throws DBException {
        try (Connection dbCon = database.openConnection();
             PreparedStatement dbStat = dbCon.prepareStatement(
                 "SELECT AUTH_ROLE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) +
                     "WHERE TOKEN_ID=?"
             )
        ) {
            dbStat.setString(1, smAccessToken);
    ...
    public void initializeMetaInformation() throws DBCException {
        try (Connection dbCon = database.openConnection()) {
            try (JDBCTransaction txn = new JDBCTransaction(dbCon)) {
                ...
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "SELECT PROVIDER_ID FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PROVIDER))) {
                    try (ResultSet dbResult = dbStat.executeQuery()) {
                        while (dbResult.next()) {
                ...
                try (PreparedStatement dbStat = dbCon.prepareStatement(
                    "INSERT INTO " +
                        database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_PROVIDER) + "(PROVIDER_ID,IS_ENABLED) VALUES(?,'Y')")) {
    ...
    private void createAuthSubject(Connection dbCon, String subjectId, String subjectType) throws SQLException {
        try (PreparedStatement dbStat = dbCon.prepareStatement("INSERT INTO " +
            database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_SUBJECT) + "(SUBJECT_ID,SUBJECT_TYPE) VALUES(?,?)")) {
            dbStat.setString(1, subjectId);
    ...
    public void clearOldAuthAttemptInfo() throws DBException {
        try (Connection dbCon = database.openConnection()) {
            JDBCUtils.executeStatement(dbCon,
                "DELETE FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT_INFO) + " AAI " +
                "WHERE EXISTS " +
                "(SELECT 1 FROM " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_ATTEMPT) + " AA " +
                "LEFT JOIN " + database.getTableName(CBDatabaseConstants.TABLE_CB_AUTH_TOKEN) + " CAT " +
                    "ON AA.SESSION_ID = CAT.SESSION_ID " +
