...
export function useHighlightNewLine(enabled: boolean): [Compartment, Extension] | null {
  const codemirror = useLazyImport(codemirrorPluginLoader);

  const HIGHLIGHT_NEW_LINE_COMPARTMENT = useMemo(() => {
    if (!codemirror) {
      return null;
    }
  ...
  return useMemo(() => {
    if (!codemirror || !HIGHLIGHT_NEW_LINE_COMPARTMENT || !enabled) {
      return null;
    }

    return [HIGHLIGHT_NEW_LINE_COMPARTMENT, codemirror.highlightNewLine()];
  }, [HIGHLIGHT_NEW_LINE_COMPARTMENT, codemirror, enabled]);
}
