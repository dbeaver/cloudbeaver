...
export const TopNavButton = observer<IProps, HTMLButtonElement>(
  forwardRef(function TopNavButton({ title, icon, secondary, menu, loading, style, ...props }, ref) {
        ...
        <box>
          {(icon || loading) && (
            <menu-trigger-icon {...use({ loading })}>
              <Loader secondary={secondary} loading={loading} small>
                {icon && <IconOrImage icon={icon} />}
              </Loader>
            </menu-trigger-icon>
          )}
