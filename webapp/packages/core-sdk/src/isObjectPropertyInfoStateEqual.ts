...
export function isObjectPropertyInfoStateEqual<T>(properties: RequiredObjectPropertyInfoFields[], first: T, second: T): boolean {
  if (first === null || second === null || typeof first !== 'object' || typeof second !== 'object') {
    return false;
  }

  const customKeys = [...Object.keys(first), ...Object.keys(second)].filter(key => !properties.some(property => key === property.id));
