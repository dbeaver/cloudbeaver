...
export const ConnectionAccess: TabContainerPanelComponent<IUserFormProps> = observer(function ConnectionAccess({ controller, editing }) {
  ...
  const getConnectionPermission = useCallback(
    (connectionId: string) => controller.grantedConnections.find(connectionPermission => connectionPermission.dataSourceId === connectionId),
  ...
  const localConnections = useMemo(
    () => computed(() => controller.connections.filter(connection => !isCloudConnection(connection))),
              ...
              return (
                <TableItem key={connection.id} item={connection.id} selectDisabled={isTeamProvided}>
                  <TableColumnValue centerContent flex>
                    <TableItemSelect disabled={loading || isTeamProvided} checked={isTeamProvided} />
                  </TableColumnValue>
                  <TableColumnValue>
