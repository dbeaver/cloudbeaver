...
import { ITab, NavigationTabsService, NavTreeResource } from '@cloudbeaver/core-app';
...
@injectable()
export class ScriptManagerSyncService {
  ...
  private async syncTab(tab: ITab<ISqlEditorTabState>) {
    try {
      ...
      const found = await this.navTreeResource.preloadNodeParents(
        [RESOURCES_NODE_PATH, this.scriptsManagerService.userProjectPath], tab.handlerState.associatedScriptId
      );

      if (!found) {
        this.notificationService.logInfo({
          title: 'plugin_resource_manager_script_not_found_title',
          message: 'plugin_resource_manager_script_not_found_message',
          persistent: true,
        });
      ...
      const query = await this.scriptsManagerService.readScript(tab.handlerState.associatedScriptId);
