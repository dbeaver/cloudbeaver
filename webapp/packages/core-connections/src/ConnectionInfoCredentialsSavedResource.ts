...
import { type DatabaseConnectionCredentialsSavedFragment, GraphQLService } from '@cloudbeaver/core-sdk';
...
export type ConnectionInfoCredentialsSaved = DatabaseConnectionCredentialsSavedFragment;

// TODO reuse it everywhere where include "includeCredentialsSaved" is used
@injectable()
export class ConnectionInfoCredentialsSavedResource extends CachedMapResource<IConnectionInfoParams, ConnectionInfoCredentialsSaved> {
  constructor(
  ...
  protected async loader(
    ...
    _: any,
    refresh?: boolean,
  ): Promise<Map<IConnectionInfoParams, ConnectionInfoCredentialsSaved>> {
    const connectionsList: ConnectionInfoCredentialsSaved[] = [];
    ...
    await ResourceKeyUtils.forEachAsync(key, async connectionKey => {
      ...
      const { connections } = await this.graphQLService.sdk.getUserConnectionsCredentialsSaved({
        projectId,
        connectionId,
        projectIds,
      });
