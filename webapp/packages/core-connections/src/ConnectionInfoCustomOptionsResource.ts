...
import { type DatabaseConnectionCustomOptionsFragment, GraphQLService } from '@cloudbeaver/core-sdk';
...
export class ConnectionInfoCustomOptionsResource extends CachedMapResource<IConnectionInfoParams, ConnectionInfoCustomOptions> {
  ...
  protected async loader(
    ...
    _: any,
    refresh?: boolean,
  ): Promise<Map<IConnectionInfoParams, ConnectionInfoCustomOptions>> {
    const connectionsList: ConnectionInfoCustomOptions[] = [];
    ...
    await ResourceKeyUtils.forEachAsync(key, async connectionKey => {
      ...
      const { connections } = await this.graphQLService.sdk.getUserConnectionsCustomOptions({
        projectId,
        connectionId,
        projectIds,
      });
