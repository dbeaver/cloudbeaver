...
import { type DatabaseConnectionAuthPropertiesFragment, GraphQLService } from '@cloudbeaver/core-sdk';
...
export class ConnectionInfoAuthPropertiesResource extends CachedMapResource<IConnectionInfoParams, ConnectionInfoAuthProperties> {
  ...
  protected async loader(
    ...
    _: any,
    refresh?: boolean,
  ): Promise<Map<IConnectionInfoParams, ConnectionInfoAuthProperties>> {
    const connectionsList: ConnectionInfoAuthProperties[] = [];
    ...
    await ResourceKeyUtils.forEachAsync(key, async connectionKey => {
      ...
      const { connections } = await this.graphQLService.sdk.getUserConnectionsAuthProperties({
        projectId,
        connectionId,
        projectIds,
      });
