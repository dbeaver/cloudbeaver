    ...
    this.menuService.addCreator({
      ...
      getItems: (context, items) => {
        ...
        const translatedLabel =
          this.localizationService.translate('plugin_navigation_tree_filters_configuration', undefined, { name: node.name }) + '...';
        const { clippedLabel, tooltip } = getMenuLabelClipped(translatedLabel);
        const actions = [
          new MenuBaseItem(
            {
              id: 'configure-filter',
